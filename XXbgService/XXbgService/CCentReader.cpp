...
// 重写，获取身份证芯片信息
int CCentReader::MyReadIDCardInfo(const char *pSaveHeadPicFilenm, MYPERSONINFO *pPersonInfo)
{
	...
	// 设备连接端口
	GetPrivateProfileString("CentermReader", "Device", "HID", sTmpString, sizeof(sTmpString)-1, sIniFilePath);
	memcpy(sDiv, (const char*)sTmpString, sizeof(sDiv)-1);
	// BP口
	GetPrivateProfileString("CentermReader", "BpNo", "9", sTmpString, sizeof(sTmpString)-1, sIniFilePath);
	memcpy(sBp, (const char*)sTmpString, sizeof(sBp)-1);
	// 串口波特率
	GetPrivateProfileString("CentermReader", "Baud", "115200", sTmpString, sizeof(sTmpString)-1, sIniFilePath);
	memcpy(sBaud, (const char*)sTmpString, sizeof(sBaud)-1);
	// 寻卡超时时间
	GetPrivateProfileString("CentermReader", "TimeOut", "5", sTmpString, sizeof(sTmpString)-1, sIniFilePath);
