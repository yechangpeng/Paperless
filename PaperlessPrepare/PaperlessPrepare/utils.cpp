...
// 用'|'分割字符串，获取第 index 个子串(index从0开始)
// 0-成功，其他-失败
int splitString(char *des, const char *src, int index)
{
	if (des == NULL || src == NULL || index < 0)
	{
		return -1;
	}
	...
	else
	{
		...
		if (curPos == NULL)
		{
			memcpy(des, prePos, strlen(prePos));
		}
		else
		{
			memcpy(des, prePos, curPos-prePos);
		}
		return 0;
	}
	return 0;
...
char* MyGetLastError(char *sLastMsg, int strLen)
{
	...
	memcpy(sLastMsg, (LPCTSTR)lpMsgBuf, strLen-1);
	LocalFree( lpMsgBuf );
	return sLastMsg;
}
...
{
	if (pStat == NULL || sFilePath.IsEmpty())
	{
		return FALSE;
	}
	wchar_t  *wchar = (wchar_t *)malloc(sizeof(wchar_t) * 1024);
	MultiByteToWideChar( CP_ACP, 0, sFilePath,  strlen(sFilePath)+1, wchar, sizeof(wchar_t) * 1024 / sizeof(wchar[0]) );
	if(0 != _wstati64(wchar, pStat))/*pFileName是文件的路径*/
	{
		free(wchar);
		return FALSE;
	}
